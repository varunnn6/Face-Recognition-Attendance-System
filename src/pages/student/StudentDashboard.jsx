...
import { CalendarDays, ScanFace, Clock, ArrowRight, CheckCircle, XCircle } from 'lucide-react';

export default function StudentDashboard() {
  const { user } = useAuth();
  const { activeSessions, loading, getAttendanceByStudent, getAttendanceStats } = useData();
      ...
      <div style={{ display: 'flex', gap: 'var(--space-xl)', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 2, minWidth: 400 }}>
          ...
          <div className="card-static" style={{ padding: 22, marginBottom: 'var(--space-xl)' }}>
            <h3 style={{ fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-dim)', marginBottom: 'var(--space-md)' }}>SUBJECT-WISE ATTENDANCE</h3>
            {subjectStats.length === 0 ? (
              ...
              <div key={i} style={{ padding: '12px 0', borderBottom: '1px solid var(--border-subtle)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                  ...
                  <span style={{ fontSize: '0.82rem', fontWeight: 700, color: sub.percentage >= 75 ? 'var(--accent-primary)' : sub.percentage >= 50 ? 'var(--accent-warning)' : 'var(--accent-danger)' }}>{sub.percentage}%</span>
                </div>
                <div style={{ height: 6, background: 'var(--border-subtle)', borderRadius: 'var(--radius-full)', overflow: 'hidden' }}>
                  <div style={{ width: `${sub.percentage}%`, height: '100%', background: sub.percentage >= 75 ? 'var(--accent-primary)' : sub.percentage >= 50 ? 'var(--accent-warning)' : 'var(--accent-danger)', borderRadius: 'var(--radius-full)', transition: 'width 0.5s ease' }} />
                </div>
                <div style={{ fontSize: '0.72rem', color: 'var(--text-dim)', marginTop: 4 }}>{sub.present}P / {sub.absent}A / {sub.total} Total</div>
              </div>
            ))}
          </div>
        </div>
